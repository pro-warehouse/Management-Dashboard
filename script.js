...
function renderOnTimeSection() {
    ...
    let affNames = Object.keys(affSource).filter(a => a && Object.keys(affSource[a] || {}).length > 0).sort();
    ...
    // จัดกลุ่มตามช่วงเวลา (Daily/Monthly) แยกรายสังกัด แล้วหาค่าเฉลี่ยต่อกลุ่ม
    let groupedAff = {}; let groupTime = {};
    selectedAffs.forEach(aff => {
        groupedAff[aff] = {};
    ...
    if (ontimeChartInstance) {
        let datasets = selectedAffs.map((aff, i) => {
            let color = chartPalette[i % chartPalette.length];
            return {
                label: aff,
                data: sliceKeys.map(gk => { let g = groupedAff[aff][gk]; return (g && g.cnt > 0) ? parseFloat((g.sum / g.cnt).toFixed(2)) : null; }),
                borderColor: color,

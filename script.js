...
const lineDataLabelPlugin = {
    ...
    afterDatasetsDraw(chart) {
        ...
        // [FIX #5] ถ้า On-Time มีหลายเส้น (หลายสังกัด) ไม่ต้องวาดป้าย % เพราะตัวเลขจะทับกันรก
        if (chart.canvas.id === 'ontimeChart2' && chart.data.datasets.length > 2) return;
        const { ctx } = chart;
        chart.data.datasets.forEach((dataset, i) => {
            if (!chart.isDatasetVisible(i)) return;
            const meta = chart.getDatasetMeta(i);
...
function renderInventorySummary() {
    ...
    const buildBlock = (title, g) => {
        if (!g) return `<div style="margin-bottom:10px;"><b>${title}:</b> <span style="color:var(--text-muted);">ไม่มีข้อมูล</span></div>`;
        let bd = g.data.bu_data || {};
        let oh=0, df=0, cnt=0; let buParts = [];
        Object.keys(bd).sort().forEach(bu => {
            if (!buPass(bu)) return;
            let d = bd[bu]; if (!d || !d.count) return;
            oh += d.onhand; df += d.diff; cnt += d.count;
            buParts.push(`<span style="white-space:nowrap;">${bu} ${pill(acc(d.onhand, d.diff))}</span>`);
        });
        let overall = cnt > 0 ? acc(oh, df) : null;

        let lt = g.data.loctype_data || {};
        let typeParts = [];
        Object.keys(lt).sort().forEach(t => {
            if (!typePass(t)) return;
            let toh=0, tdf=0, tcnt=0;
            if (lt[t].bu && Object.keys(lt[t].bu).length) {
                Object.keys(lt[t].bu).forEach(bu => { if (buPass(bu)) { toh += (lt[t].bu[bu].onhand||0); tdf += (lt[t].bu[bu].diff||0); tcnt += (lt[t].bu[bu].count||0); } });
            } else {
                toh = lt[t].onhand||0; tdf = lt[t].diff||0; tcnt = lt[t].count||0;
            }
            if (tcnt > 0) typeParts.push(`<span style="white-space:nowrap;">${t} ${pill(acc(toh, tdf))}</span>`);
        });

        let html = `<div style="margin-bottom:2px;"><b>${title}:</b> &nbsp; ภาพรวม ${pill(overall)} &nbsp;<span style="color:var(--text-muted);">|</span>&nbsp; ${buParts.length ? buParts.join(' &nbsp; ') : '<span style="color:var(--text-muted);">-</span>'}</div>`;
        html += `<div style="margin:0 0 12px 12px;"><span style="color:var(--text-muted);">ตาม Location Type:</span> ${typeParts.length ? typeParts.join(' &nbsp; ') : '<span style="color:var(--text-muted);">— (redeploy AppScript เพื่อดูแยก Type รายวัน)</span>'}</div>`;
        return html;
    };

    let monthG = monthArr.length ? monthArr[monthArr.length - 1] : null;
    let dayG = dayArr.length ? dayArr[dayArr.length - 1] : null;
    let mTitle = monthG ? `📅 รายเดือน (ล่าสุด ${monthG.label})` : '📅 รายเดือน';
    let dTitle = dayG ? `🗓️ รายวัน (ล่าสุด ${String(new Date(dayG.ts).getDate()).padStart(2,'0')} ${months[new Date(dayG.ts).getMonth()]})` : '🗓️ รายวัน';
    box.innerHTML = buildBlock(mTitle, monthG) + buildBlock(dTitle, dayG);
}

...
let globalData = { workforce:{}, fulfillment:{}, wave_ops:{}, ontime:{}, ontime_hub:{}, ontime_by_aff:{}, claims:{}, inventory:{}, inventory_daily:{}, transport:{}, productivity:{}, prod_area:{}, prod_users_map:{} };
        ...
        Object.values(data.matrix[aff][r]).forEach(val => sum += val);
            ...
            if (fgEl) {
                ...
                if (fgList.length === 0) {
                    ...
                    fgList.forEach(f => {
                        let k = (f.aff||'-') + '|' + (f.role||'-');
                        if (!grp[k]) grp[k] = { aff: f.aff||'-', role: f.role||'-', names: [] };
                        grp[k].names.push(f.nickname ? `${f.name} (${f.nickname})` : f.name);
                    });
                    // [FIX] ไม่โชว์รายชื่อ (ข้อมูลเยอะ) — โชว์แค่ สังกัด / ตำแหน่ง / จำนวน
                    let rows = Object.values(grp).sort((a,b) => (a.aff+a.role).localeCompare(b.aff+b.role)).map(g =>
                        `<tr>
                            ...
                            <td style="padding:6px 10px; border-bottom:1px solid var(--border-color); text-align:center; font-weight:800; color:#3B82F6;">${g.names.length}</td>
                        </tr>`
                    ).join('');
                    fgEl.innerHTML = `<div style="margin-top:15px;">
                        <div style="font-size:0.8rem; font-weight:700; color:#3B82F6; margin-bottom:8px;">🌐 พนักงานต่างชาติ — แยกตามสังกัด & ตำแหน่ง (รวม ${fgList.length} คน)</div>
        ...
        if (areaTableEl) {
            ...
            if(sortedAreas.length === 0) {
                ...
                sortedAreas.forEach(a => {
                    ...
                    let trg = getTarget(a, latestDateStr);
                    let gap = uph - trg;

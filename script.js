...
if (document.getElementById('stage-pick-pct')) {
    ...
    let warningBadge = !isDataIncomplete ? "" : ` <span style="background:#fef3c7; color:#92400e; padding:2px 6px; border-radius:4px; font-size:9px; margin-left:auto;">⚠️ ใช้ข้อมูลสำรอง</span>`;
                ...
                if (fgList.length === 0) {
                    ...
                    let rows = Object.values(grp).sort((a,b) => (a.aff+a.role).localeCompare(b.aff+b.role)).map(g =>
                        `<tr>
                            ...
                            <td style="padding:6px 10px; border-bottom:1px solid var(--border-color); font-size:0.72rem; color:var(--text-muted);">${g.names.join(', ')}</td>
                        </tr>`
                    ).join('');
                    fgEl.innerHTML = `<div style="margin-top:15px;">
                        <div style="font-size:0.8rem; font-weight:700; color:#3B82F6; margin-bottom:8px;">🌐 พนักงานต่างชาติ — แยกตามสังกัด & ตำแหน่ง (${fgList.length} คน)</div>
                        <div style="overflow-x:auto;"><table style="width:100%; border-collapse:collapse; font-size:0.8rem;">
                            <thead><tr>
                                <th style="text-align:left; padding:6px 10px; color:var(--text-muted); font-size:0.7rem; border-bottom:2px solid var(--border-color);">สังกัด</th>
                                <th style="text-align:left; padding:6px 10px; color:var(--text-muted); font-size:0.7rem; border-bottom:2px solid var(--border-color);">ตำแหน่ง</th>
                                <th style="text-align:center; padding:6px 10px; color:var(--text-muted); font-size:0.7rem; border-bottom:2px solid var(--border-color);">จำนวน</th>
                                <th style="text-align:left; padding:6px 10px; color:var(--text-muted); font-size:0.7rem; border-bottom:2px solid var(--border-color);">รายชื่อ</th>
                            </tr></thead>
                            <tbody>${rows}</tbody>
                        </table></div></div>`;
                }

...
// 🟢 [ใหม่] เก็บ UPH TARGET + COST BASE ราย target_date (โครงเดียวกับ globalCapacities) เพื่อแก้ย้อนหลัง + ยึดค่าล่าสุด
let globalUphCost = {};

// 🔔 [FIX #1] Toast Notification
// เดิมโค้ดเรียกใช้ showToast(...) แต่ "ไม่เคยประกาศฟังก์ชันนี้ไว้เลย"
// ทำให้ตอนกด Save เกิด ReferenceError -> popup ไม่ขึ้น และโค้ดหลังจากนั้นไม่ทำงาน
...
function showToast(message, type = 'success', duration = 3000) {
    ...
    const shadows = {
        ...
        info:    'rgba(245, 158, 11, 0.3)'
    };
    toast.style.setProperty('background', colors[type] || colors.success, 'important');
    toast.style.setProperty('box-shadow', `0px 15px 40px ${shadows[type] || shadows.success}`, 'important');

    // แสดงผลทันที (บังคับ reflow ให้ transition ทำงานแม้เรียกซ้ำติดกัน)
...
async function saveDailyCapacity() {
    ...
    // 🌟 [FIX #1] แจ้งผู้ใช้ทันทีว่ากำลังบันทึก (แก้อาการ "ขึ้นช้า")
    //    เดิมต้องรอ backend บน Render ตอบกลับก่อน (ซึ่งบางครั้ง cold start นานหลายวินาที)
    //    จึงค่อยเด้ง toast ทำให้รู้สึกว่าช้า/ไม่ขึ้น -> ตอนนี้เด้งทันทีที่กด
    ...
    showToast('⏳ กำลังบันทึก Capacity...', 'info', 0); // duration 0 = ค้างไว้จนกว่าจะมีผลลัพธ์

    // รวบรวมข้อมูลทุก BU ให้เป็นก้อนเดียว (Array)
    let capacityData = [];
    ...
    try {
        ...
        // อัปเดตค่าในหน้าจอทันที
        if (!globalCapacities[targetDate]) globalCapacities[targetDate] = {};
        for (let item of capacityData) {
            globalCapacities[targetDate][item.owner] = item.capacity;
        }

        showToast(`✅ บันทึก Capacity ของวันที่ ${targetDate} สำเร็จแล้ว!`, 'success');
        ...
        if (saveBtn) { saveBtn.disabled = false; saveBtn.innerHTML = originalBtnHtml || '💾 Save Capacity'; }
    }
}
// === Setup Base & Theme ===
...
themeToggleBtn.addEventListener('click', () => {
    ...
    // เปลี่ยนสีตัวหนังสือของแกนกราฟตาม Theme
...
// กำหนดชุดสีหลักให้เป็น Solid Color ตาม Reference
const accentGreen = '#10B981';
const accentOrange = '#F59E0B';
const dangerRed = '#EF4444';
const primaryBlue = '#3B82F6';

// ชุดสีสำหรับแยกตาม BU (เน้นโทนสีที่ตัดกันชัดเจนบนพื้นขาว)
const chartPalette = [
    '#3B82F6', '#10B981', '#F59E0B', '#8B5CF6', '#F43F5E', 
    '#06B6D4', '#14B8A6', '#84CC16', '#A855F7', '#EF4444'
];

const fmtN = (v) => (v || 0).toLocaleString();

// 🛠️ 1. Smart Date Normalizer
...
// ==========================================
// 🌟 [FIX #5/#6/#7] Reusable Multi-Select Dropdown (เลือกได้หลายรายการ)
// ใช้กับ: On-Time (แยกสังกัด), Claim (BU), Inventory (Location Type)
// mountEl = <div> ว่างใน HTML | opts = { label, onApply(selectedArray) }
// คืน object: { getSelected(), setOptions(arr, keepSelection), el }
//   - getSelected() คืน ['ALL'] เมื่อเลือกครบทุกตัวหรือไม่เลือกเลย, ไม่งั้นคืน array ที่เลือก
// ==========================================
...
function createMultiSelect(mountEl, opts) {
    ...
    mountEl.innerHTML = `
        <div class="ms-btn" style="cursor:pointer; min-width:110px; display:flex; justify-content:space-between; align-items:center; gap:8px; padding:4px 10px; font-size:0.8rem; border:1px solid var(--border-color); border-radius:8px; background:var(--bg-card); color:var(--text-main);">
            <span class="ms-text" style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; max-width:140px; font-weight:700;">${label}: All</span>
            <span style="font-size:10px; color:var(--text-muted);">▼</span>
        </div>
        <div class="ms-menu" style="display:none; position:absolute; top:100%; right:0; min-width:175px; background:var(--bg-card); border:1px solid var(--border-color); border-radius:10px; box-shadow:var(--shadow-card); z-index:999; max-height:260px; overflow-y:auto; padding:12px; margin-top:6px;">
            <label style="display:flex; align-items:center; gap:6px; font-weight:700; cursor:pointer; font-size:0.8rem;"><input type="checkbox" class="ms-all" checked> All</label>
            <div style="border-bottom:1px dashed var(--border-color); margin:8px 0;"></div>
            <div class="ms-list" style="display:flex; flex-direction:column; gap:6px;"></div>
            <button class="ms-apply" style="margin-top:10px; width:100%; padding:6px; background:#114EE8; color:white; border:none; border-radius:6px; cursor:pointer; font-weight:600; font-size:0.8rem;">Apply</button>
        </div>
    `;
    ...
    const renderList = () => {
        ...
        options.forEach(v => {
            ...
            lbl.style.cssText = 'display:flex; align-items:center; gap:6px; cursor:pointer; color:var(--text-main); font-size:0.78rem;';
    ...
    return {
        ...
        setOptions(arr, keepSelection = true) {
            let newOpts = Array.from(new Set(arr || [])).filter(v => v !== undefined && v !== null && v !== '');
            newOpts.sort();
            let wasAll = isAll() || selected.size === 0;
            options = newOpts;
            if (!keepSelection || wasAll) {
...
// ==========================================
// 🌟 1. Custom Plugins (ป้ายกำกับบนกราฟ) 🌟
// ==========================================
const dataLabelPlugin = {
    ...
    afterDatasetsDraw(chart) {
        ...
        // วาดผลรวม (Grand Total) ไว้บนยอดของกราฟ Stacked Bar (ffmTrendChart + workforceChart [FIX #4])
        if (chart.canvas.id === 'ffmTrendChart' || chart.canvas.id === 'workforceChart') {
            ...
            } catch(e) { console.warn("Grand Total Drawing Error:", e); }
            return; 
        }

        // วาดป้ายตัวเลขบนกราฟ Bar อื่นๆ
        chart.data.datasets.forEach((dataset, i) => {
...
const lineDataLabelPlugin = {
    ...
    afterDatasetsDraw(chart) {
        ...
        const { ctx } = chart;
        // 🟢 [FIX] On-Time: โชว์ตัวเลข % บนกราฟเสมอ — แต่ถ้ามีหลายเส้น (>3) ให้โชว์เฉพาะเส้น "ภาพรวม" กันตัวเลขทับกันรก
...
if (orderMixCtx) new Chart(orderMixCtx, { type: 'doughnut', data: { labels: ['E-com', 'B2B', '3PL'], datasets: [{ data: [55, 30, 15], backgroundColor: [accentGreen, accentOrange, primaryBlue], borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false, cutout: '75%', plugins:{legend:{position: 'bottom'}} } });

let workforceChartInstance = null;
const wfCtx = document.getElementById('workforceChart')?.getContext('2d');
if (wfCtx) workforceChartInstance = new Chart(wfCtx, { type: 'bar', data: { labels: [], datasets: [{ label: 'จำนวนกำลังพล (คน)', data: [], backgroundColor: primaryBlue, borderRadius: 6 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { display: false } }, y: { beginAtZero: true, border: { display: false } } }, layout: { padding: { top: 20 } } }, plugins: [dataLabelPlugin] });
...
if (ffmTrendCtx) ffmTrendChartInstance = new Chart(ffmTrendCtx, { 
    ...
    options: { 
        ...
        scales: { 
            ...
            y: { stacked: true, border: { display: false }, beginAtZero: true, grace: '15%' } 
        } 
    } 
});
...
if (claim1Ctx) claimChart1Instance = new Chart(claim1Ctx, { type: 'bar', data: { labels: [], datasets: [ { label: 'มูลค่าเคลม (฿)', data: [], backgroundColor: dangerRed, borderRadius: 4, yAxisID: 'y' }, { label: 'จำนวนชิ้น (Pcs)', data: [], borderColor: primaryBlue, backgroundColor: primaryBlue, type: 'line', tension: 0.4, yAxisID: 'y1', pointRadius: 2 } ] }, options: { responsive: true, maintainAspectRatio: false, plugins:{legend:{display:false}}, scales: { x: {grid:{display:false}}, y: {position: 'left', beginAtZero: true}, y1: {position: 'right', beginAtZero: true, grid: {display:false}, display: false} }, layout: { padding: { top: 10 } } }, plugins: [dataLabelPlugin] });

let claimChart2Instance = null;
const claim2Ctx = document.getElementById('claimChart2')?.getContext('2d');
if (claim2Ctx) claimChart2Instance = new Chart(claim2Ctx, { type: 'bar', data: { labels: [], datasets: [ { label: 'มูลค่าเคลม (฿)', data: [], backgroundColor: dangerRed, borderRadius: 6, yAxisID: 'y' }, { label: 'จำนวนชิ้น (Pcs)', data: [], borderColor: primaryBlue, backgroundColor: primaryBlue, type: 'line', tension: 0.4, yAxisID: 'y1', pointRadius: 4, borderWidth: 2 } ] }, options: { responsive: true, maintainAspectRatio: false, plugins:{ legend:{ display:true, position: 'top', labels: { padding: 20, boxWidth: 12, font: {size: 11} } } }, scales: { x: {grid:{display:false}}, y: {position: 'left', beginAtZero: true, grace: '25%', title: {display: true, text: 'มูลค่ารวม (บาท)', font: {size: 10}}}, y1: {position: 'right', beginAtZero: true, grace: '25%', grid: {display:false}, title: {display: true, text: 'จำนวนสินค้า (ชิ้น)', font: {size: 10}}} }, layout: { padding: { top: 10, left: 10, right: 10 } } }, plugins: [dataLabelPlugin] });

let ontimeChartInstance = null;
const otChartEl = document.getElementById('ontimeChart2');
if (otChartEl) ontimeChartInstance = new Chart(otChartEl.getContext('2d'), { type: 'line', data: { labels: [], datasets: [ { label: 'PTGLG %', data: [], borderColor: accentGreen, backgroundColor: 'rgba(16, 185, 129, 0.1)', fill: true, tension: 0.4, pointBackgroundColor: '#FFFFFF', pointBorderColor: accentGreen, pointRadius: 4, spanGaps: true }, { label: 'HUB %', data: [], borderColor: primaryBlue, backgroundColor: 'transparent', fill: false, tension: 0.4, pointBackgroundColor: '#FFFFFF', pointBorderColor: primaryBlue, borderDash: [5, 5], pointRadius: 4, spanGaps: true } ] }, options: { responsive: true, maintainAspectRatio: false, plugins:{legend:{display:true, position: 'bottom'}}, scales: { x: {grid:{display:false}}, y: { border:{display:false} } }, layout: { padding: { top: 40 } } }, plugins: [lineDataLabelPlugin] });
...
// ========================================================
// 🟢 [FIX #B1] Cap แบบ "ยึดค่าล่าสุด" (carry-forward)
//   ถ้าวันที่ที่ดูยังไม่ได้ตั้ง Cap ให้ใช้ Cap ของวันล่าสุด "ก่อนหรือเท่ากับ" วันนั้นแทน DEFAULT
// ========================================================
function _tsOf(dStr){ let t = new Date(dStr).getTime(); return isNaN(t) ? 0 : t; }

function hasEffectiveCap(dateStr, bu) {
    if (!bu || !dateStr) return false;
    let ref = _tsOf(dateStr);
    return Object.keys(globalCapacities).some(d =>
        globalCapacities[d] && globalCapacities[d][bu] != null && _tsOf(d) <= ref);
}
function getEffectiveCap(dateStr, bu) {
    if (!bu || !dateStr) return DEFAULT_CAPACITY;
    let ref = _tsOf(dateStr);
    let cand = Object.keys(globalCapacities)
        .filter(d => globalCapacities[d] && globalCapacities[d][bu] != null && _tsOf(d) <= ref)
        .sort((a,b) => _tsOf(b) - _tsOf(a));
    return cand.length > 0 ? globalCapacities[cand[0]][bu] : DEFAULT_CAPACITY;
}

// ========================================================
// 🟢 [FIX #F1] UPH TARGET + COST BASE แบบ "ยึดค่าล่าสุด" (เก็บบน BigQuery เหมือน Cap)
// ========================================================
function _uphDefaults() {
    return {
        trg_all:  parseFloat(document.getElementById('trg-all')?.value)  || 150,
        trg_full: parseFloat(document.getElementById('trg-full')?.value) || 150,
        trg_half: parseFloat(document.getElementById('trg-half')?.value) || 150,
        trg_ea:   parseFloat(document.getElementById('trg-ea')?.value)   || 150,
        cost_salary: parseFloat(document.getElementById('cost-salary')?.value) || 15000,
        cost_days:   parseFloat(document.getElementById('cost-days')?.value)   || 26
    };
}
// คืนค่า target/cost ที่ "มีผล ณ วันที่" (carry-forward); ถ้าไม่เคยตั้งเลย ใช้ค่าใน input เป็น default
function getEffectiveUphCost(dateStr) {
    let def = _uphDefaults();
    if (!dateStr) dateStr = document.getElementById('date-picker')?.value || new Date().toISOString().split('T')[0];
    let ref = _tsOf(dateStr);
    let cand = Object.keys(globalUphCost).filter(d => _tsOf(d) <= ref).sort((a,b) => _tsOf(b) - _tsOf(a));
    if (cand.length === 0) return def;
    let r = globalUphCost[cand[0]];
    return {
        trg_all:  r.trg_all  != null ? parseFloat(r.trg_all)  : def.trg_all,
        trg_full: r.trg_full != null ? parseFloat(r.trg_full) : def.trg_full,
        trg_half: r.trg_half != null ? parseFloat(r.trg_half) : def.trg_half,
        trg_ea:   r.trg_ea   != null ? parseFloat(r.trg_ea)   : def.trg_ea,
        cost_salary: r.cost_salary != null ? parseFloat(r.cost_salary) : def.cost_salary,
        cost_days:   r.cost_days   != null ? parseFloat(r.cost_days)   : def.cost_days
    };
}
// เติมค่าใน input ให้ตรงกับค่าที่ "มีผล" ของวันที่ที่เลือกแก้ (ให้ผู้ใช้เห็นค่าปัจจุบันก่อนแก้)
function applyUphCostInputs(dateStr) {
    let dEl = document.getElementById('uph-target-date');
    if (!dateStr) dateStr = dEl?.value || document.getElementById('date-picker')?.value || new Date().toISOString().split('T')[0];
    if (dEl && dateStr) dEl.value = dateStr;
    let eff = getEffectiveUphCost(dateStr);
    const set = (id, v) => { let el = document.getElementById(id); if (el) el.value = v; };
    set('trg-all', eff.trg_all); set('trg-full', eff.trg_full); set('trg-half', eff.trg_half); set('trg-ea', eff.trg_ea);
    set('cost-salary', eff.cost_salary); set('cost-days', eff.cost_days);
}
// ดึง UPH/Cost ที่บันทึกไว้จาก backend มาเก็บใน globalUphCost
async function fetchUphCost(startDate, endDate) {
    try {
        const resp = await fetch("https://dc-ordermonitoring-backend.onrender.com/api/run", {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ fn: 'apiGetUphCost', args: [startDate, endDate] })
        });
        if (resp.ok) {
            const j = await resp.json();
            if (j.success && Array.isArray(j.data)) {
                j.data.forEach(r => {
                    let d = (r.target_date && r.target_date.value) ? r.target_date.value : r.target_date;
                    if (d) globalUphCost[d] = { ...r, target_date: d };
                });
            }
        }
    } catch (e) { console.warn("⚠️ ดึง UPH/Cost ไม่สำเร็จ ใช้ค่า default แทน", e); }
}
...
async function initFulfillmentRealtime() {
    ...
    if (!wrapperEl) {
        ...
        if (!tableEl) return;
        const targetCard = tableEl.closest('.data-card, .card, .chart-card') || tableEl.parentElement;
    ...
    try {
        ...
        // 🟢 [FIX #B1] ดึงย้อนหลัง 90 วัน เพื่อให้มีประวัติ Cap มากพอสำหรับ "ยึดค่าล่าสุด" (carry-forward)
        ...
        // 🟢 [FIX #F1] ดึง UPH TARGET / COST BASE ที่บันทึกไว้ (ช่วงเดียวกับ Cap) มาเก็บใน globalUphCost
        await fetchUphCost(startDate, dpVal);
    ...
    try {
            ...
            try { 
                ...
                Object.keys(rawBq).forEach(rawBu => {
                    ...
                    if (sBu !== 'UNKNOWN' && sBu !== '') {
                        ...
                        // 🌟 [FIX #3] ใช้ค่า ordFull จาก BigQuery ตรงๆ (BigQuery นับ "order ที่จัดครบ 100% ไม่ขาดแม้ชิ้นเดียว" มาให้แล้ว)
                        //    เดิมมี fallback: ถ้ายอด ship รวม >= req รวม จะ "เหมา" ว่าทุก order สมบูรณ์
                        //    ซึ่งผิดหลัก: order นึง over-ship อีก order นึงขาด ยอดรวมอาจ >= req แต่ order ที่ขาดต้องได้ 0
                        //    -> ตัด fallback ออก เพื่อไม่ให้ Ord.SLA พองเกินจริง
        ...
        // 🟢 [FIX #B2] ตาราง MATRIX แสดงเฉพาะ 14 วันย้อนหลัง (นับรวมวันที่เลือก) — แต่ยัง build chartDataMap ครบทุกวันเพื่อไม่ให้กราฟ/การ์ดพัง
        ...
        if (sortedDates.length === 0) {
            ...
            sortedDates.forEach(dateStr => {
                ...
                sortedOwners.forEach(bu => {
                    ...
                    // 🌟 [FIX #3] Ord.SLA = Perfect Order Rate = (order ที่จัดครบ 100%) / (order ทั้งหมด)
                    //    เงื่อนไข: order ที่ขาดแม้แต่ชิ้นเดียว = 0% ทันที และค่าห้ามเกิน 100%
                    //    ต้องคิดจาก "แหล่งเดียว" คือ actual ของ BigQuery (bItem.ordFull/ordTotal)
                    //    ซึ่งนับ order ที่ไม่ขาดเลยมาให้แล้ว — ไม่เอา completed ของ wave/ffm (= 'ปิดงาน' ไม่ใช่ 'ของครบ') มาปน
                    //    ถ้าไม่มีข้อมูล actual ของวันนั้น ให้โชว์ '-' แทนการเดา (ดีกว่าโชว์ 100% ลอยๆ)
                    ...
                    if (inMatrixWindow) htmlTable += `<tr style="cursor: pointer; color:var(--text-main); white-space:nowrap;" onmouseover="this.style.backgroundColor='var(--bg-body)'" onmouseout="this.style.backgroundColor='transparent'">
        ...
        // 🟢 [FIX #B2] ถ้ามีข้อมูลแต่ไม่มีวันไหนอยู่ใน 14 วันล่าสุด ให้แจ้งแทนตารางว่าง
        ...
        try {
            if (typeof ffmTrendChartInstance !== 'undefined' && ffmTrendChartInstance && validChartDates.length > 0) {
                ...
                const trendTitle = document.querySelector('#ffmTrendChart')?.closest('.card, .data-card, .chart-card')?.querySelector('h3');
                ...
                let dailyCapData = validChartDates.map(dStr => {
                    ...
                    chartBUsArray.forEach(bu => {
                        // 🟢 [FIX #B1] ยึด Cap ล่าสุดที่กำหนดไว้ (carry-forward) แทนการ reset เป็น DEFAULT ทุกวัน
                        dayTotalCap += getEffectiveCap(dStr, bu);
                    });
                    return dayTotalCap;
                });
                ...
                ffmTrendChartInstance.options.scales.y.grace = '15%'; 
            ...
            if (typeof ffmVolumeChartInstance !== 'undefined' && ffmVolumeChartInstance && validChartDates.length > 0) {
                ...
                const volTitle = document.querySelector('#ffmVolumeChart')?.closest('.card, .data-card, .chart-card')?.querySelector('h3');
                if (volTitle) {
        ...
        if (utilTableEl && sortedDates.length > 0) {
            ...
            chartBUsArray.forEach(bu => {
                ...
                // 🟢 [FIX #B1] ใช้ Cap ที่มีผล ณ วันนั้น (ยึดค่าล่าสุด) แทน DEFAULT ถ้าวันนั้นยังไม่ได้ตั้ง
            ...
            if (utilData.length === 0) {
                ...
                utilData.forEach(item => {
                    // 🌟 [FIX #2] ห้ามให้ตัวเลข UTILIZATION % ทะลุเกิน 100%
                    //    (เดิมโชว์ค่าดิบ เช่น 150% -> cap ที่ 100% ส่วนสถานะ "Overloaded" ยังบอกว่าเกินอยู่)
        ...
        if (ordersEl) {
            ...
            if (trendEl) {
                ...
                if (totalOrdersYesterday === 0 && totalOrdersToday === 0) {
                    trendEl.style.background = "rgba(43, 54, 116, 0.05)"; trendEl.style.color = "var(--text-main)"; trendEl.innerText = "-"; if(trendTextEl) trendTextEl.innerText = "ไม่มีข้อมูลเปรียบเทียบ";
                } else if (totalOrdersYesterday === 0) {
                    trendEl.style.background = "rgba(16, 185, 129, 0.1)"; trendEl.style.color = "#10B981"; trendEl.innerText = "↗ 100%"; if(trendTextEl) trendTextEl.innerText = `vs ${prevDateText}`;
                } else {
                    let pctDiff = ((totalOrdersToday - totalOrdersYesterday) / totalOrdersYesterday) * 100;
                    if (pctDiff > 0) { trendEl.style.background = "rgba(16, 185, 129, 0.1)"; trendEl.style.color = "#10B981"; trendEl.innerText = `↗ +${pctDiff.toFixed(1)}%`; } 
                    else if (pctDiff < 0) { trendEl.style.background = "rgba(239, 68, 68, 0.1)"; trendEl.style.color = "#EF4444"; trendEl.innerText = `↘ ${Math.abs(pctDiff).toFixed(1)}%`; } 
                    else { trendEl.style.background = "rgba(43, 54, 116, 0.05)"; trendEl.style.color = "var(--text-main)"; trendEl.innerText = `0%`; }
        ...
        if (rateValEl && latestShipDateStr) {
            ...
            // 🟢 [FIX] DC SLA (ETA) ต้องไม่เกิน 100% (กันกรณี over-ship ทำให้ทะลุ 100%)
            ...
            if (rateNoteEl) {
                if (!prevShipDateStr) {
                    if (rateTrendEl) { rateTrendEl.style.background = "rgba(43, 54, 116, 0.05)"; rateTrendEl.style.color = "var(--text-main)"; rateTrendEl.innerText = "-"; }
                    ...
                    if (rateTrendEl) {
                        ...
                        if (diffFfm > 0) { rateTrendEl.style.background = "rgba(16, 185, 129, 0.1)"; rateTrendEl.style.color = "#10B981"; rateTrendEl.innerText = `↗ +${diffFfm.toFixed(1)}%`; } 
                        else if (diffFfm < 0) { rateTrendEl.style.background = "rgba(239, 68, 68, 0.1)"; rateTrendEl.style.color = "#EF4444"; rateTrendEl.innerText = `↘ ${Math.abs(diffFfm).toFixed(1)}%`; } 
                        else { rateTrendEl.style.background = "rgba(43, 54, 116, 0.05)"; rateTrendEl.style.color = "var(--text-main)"; rateTrendEl.innerText = `0%`; }
                    }
        ...
        if (activeWaveKey) {
            ...
            if (document.getElementById('wave-total')) {
                ...
                // (โค้ดเก่า) อัปเดตกล่องที่ 4 ของเดิม
                ...
                // 🟢 [FIX #A2] เดิม: ถ้า API ตอบกลับ (isApiSuccess) จะยึดค่าจาก API อย่างเดียว
                //   ปัญหา: ถ้า API รายงาน late = 0 (เช่น ถามผิดวัน/สถานะเป็น done หมด) แต่ข้อมูลสรุป (GAS = aLate)
                //   เห็นว่ามีบิลหลุด SLA อยู่ กล่อง LATE ORDERS เลยขึ้น 0 ทั้งที่ยอดรวมมีดีเลย์
                //   แก้: ยึด "ค่าที่มากกว่า" ระหว่าง API กับ GAS เสมอ เพื่อไม่ให้พลาดบิลที่ช้าจริง
                ...
                // 🟢 [FIX #A2] รวมค่าดีเลย์จาก GAS (aDelay) เป็นขั้นต่ำด้วย เพื่อไม่ให้ CURRENT STATUS ขึ้น 0h0m ทั้งที่ยังค้าง
                ...
                if (document.getElementById('wave-late')) {
                    ...
                    if (isApiSuccess && apiTotalLate > 0) {
                        ...
                        document.getElementById('wave-active-info-3').innerHTML = totalLate > 0
                            ? `<span class="badge down">หลุด SLA</span> แผน ${activeDateStr} &bull; ${fmtN(totalLate)} บิล`
                            : `<span class="badge up">On-time</span> ไม่มีบิลช้ากว่าแผน`;
                    }
                }

                // อัปเดตกล่องที่ 4: CURRENT STATUS (Pick / Load)
                if (document.getElementById('wave-delay')) {
                    ...
                    if (isApiSuccess) {
                        ...
                        else if (!isNaN(pEarly) && pEarly !== null) { pStr = `<span style="color:#10B981; font-weight:700;">Early ${formatTime(pEarly)}</span>`; }
                        ...
                        else if (!isNaN(lEarly) && lEarly !== null) { lStr = `<span style="color:#10B981; font-weight:700;">Early ${formatTime(lEarly)}</span>`; }
                    ...
                    // 🟢 [FIX #A2] ถ้าแหล่งใดเห็นว่ามีดีเลย์ (รวม aDelay จาก GAS) ให้กล่องนี้โชว์ดีเลย์รวมเสมอ
                    //   กันเคส Pick/Load ของ API = done หมด แต่จริงๆ ยังมีบิลค้างเลยเวลาอยู่
                    if (maxOverallDelay > 0) { overallMainText = formatTime(maxOverallDelay); overallColor = "var(--danger)"; }
...
if (document.getElementById('stage-pick-pct')) {
    ...
    // 🟢 [FIX] ใช้ข้อมูลจริงจาก wave API (BigQuery) ทันทีที่ API ตอบกลับและมี total_orders > 0
    //   เดิมมีเงื่อนไข "< 50% ของ fTotal" ทำให้ตกไปใช้ข้อมูลสำรอง (aComp) บ่อย -> PICK=QC=SHIP เท่ากัน ดูเหมือนข้อมูลนิ่ง/จำลอง
    //   *ต้อง deploy server.js ตัวใหม่ (ที่แก้ให้ query ด้วย Planned_Pick_Date) ขึ้น Render ก่อน ค่าถึงจะเป็นของจริง*
    let isDataIncomplete = (isApiSuccess === false);
    let warningBadge = !isDataIncomplete ? "" : ` <span style="background:#fef3c7; color:#92400e; padding:2px 6px; border-radius:4px; font-size:9px; margin-left:auto;">⚠️ backend หลับ/ใช้ข้อมูลสำรอง</span>`;
    ...
    // 🚨 ดึงข้อมูล QC จาก API ที่เราเพิ่งเขียน (ถ้า API ร่วงให้ใช้ Shipped เป็น Fallback)
    let fQcDone = isDataIncomplete ? fShipped : (parseInt(waveStats.qc_orders) || 0);

    // ป้องกันค่าเพี้ยน (QC ไม่ควรเกินจำนวนที่หยิบ, ส่งออกไม่ควรเกินจำนวน QC)
    fPicked = Math.min(fPicked, fTotal);
    fQcDone = Math.min(fQcDone, fPicked); 
    fShipped = Math.min(fShipped, fQcDone); 
    
    // สิ่งที่หยิบมาแล้ว แต่ยังไม่ผ่าน QC คือของที่ค้างให้แผนก QC ตรวจ
    ...
    // 🔎 2. QC
    // % QC เทียบจาก "บิลทั้งหมดที่ต้องทำ" ไม่ใช่บิลที่หยิบมาแล้ว เพื่อให้เป็นไปในทิศทางเดียวกัน
    ...
    // แจ้งเตือนถ้ามีออเดอร์ที่เลยกำหนดเวลา QC (30 นาทีก่อนโหลด)
    let lateQcOrders = parseInt(waveStats.late_qc_orders) || 0;
    let qcAlert = lateQcOrders > 0 ? `<span style="color:#EF4444; font-weight:800; margin-left:8px;">(ดีเลย์ ${fmtN(lateQcOrders)} บิล)</span>` : ``;
    document.getElementById('stage-qc-text').innerHTML = `🔍 ค้างตรวจ/รอแพ็ค: <b>${fmtN(fQcPending)}</b> บิล` + qcAlert + warningBadge;

    // 🚚 3. SHIPPED
    let pctShip = fTotal > 0 ? ((fShipped / fTotal) * 100).toFixed(1) : 0;
    let pendingShip = Math.max(0, fTotal - fShipped); // ยังไม่ส่ง = Total - ส่งแล้ว
    ...
    document.getElementById('stage-ship-text').innerHTML = `📦 คงเหลือยังไม่ส่งออก: <b>${fmtN(pendingShip)}</b> บิล` + shipAlert + warningBadge;
}
                ...
                if (targetWaveDate) {
                    ...
                    // 🟢 [FIX #A1] เพิ่มวันที่ของข้อมูลบนกล่อง PICK / QC / SHIPPED ด้วย
...
// [FIX #5/#6/#7] สร้าง Multi-Select 3 ตัว (ตัว element อยู่ใน index.html)
ontimeAffMS = createMultiSelect(document.getElementById('ontime-aff-ms'), { label: 'สังกัด', onApply: () => renderOnTimeSection() });
claimBuMS   = createMultiSelect(document.getElementById('claim-bu-ms'),   { label: 'BU',     onApply: () => renderClaimSection() });
invLocTypeMS = createMultiSelect(document.getElementById('inv-loctype-ms'), { label: 'Type',  onApply: () => renderInventorySection() });
...
async function initDashboard() {
    ...
    if (dp && isFirstLoad) {
        ...
        dp.addEventListener('change', (e) => {
            ...
            renderInventorySection();
            renderTransportSection();
            applyUphCostInputs(dStr); // 🟢 [FIX #F1] เติมค่า UPH/Cost ที่มีผล ณ วันที่เลือก ลงในช่องกรอก
            renderProductivitySection();
            initFulfillmentRealtime();
        });
        isFirstLoad = false;
    ...
    // 🟢 [FIX #F1] initFulfillmentRealtime ดึง UPH/Cost มาแล้ว -> เติมค่าลง input ตามวันที่ + render Productivity ใหม่ให้ตรง
    applyUphCostInputs();
    renderProductivitySection();
    ...
    if(data.matrix[aff][r]) {
        Object.values(data.matrix[aff][r]).forEach(val => sum += val);
            ...
            if(hcTotalEl){
                ...
                if (trendEl && noteEl) {
                    if (opsTotal === 0 && opsYesterday === 0) {
                        trendEl.style.background = "rgba(43, 54, 116, 0.05)"; trendEl.style.color = "var(--text-main)"; trendEl.innerText = `-`; noteEl.innerText = "ไม่มีข้อมูล";
                    } else if (diff > 0) { 
                        trendEl.style.background = "rgba(16, 185, 129, 0.1)"; trendEl.style.color = "#10B981"; trendEl.innerText = `↗ +${diff}`; noteEl.innerText = vsText; 
                    } else if (diff < 0) { 
                        trendEl.style.background = "rgba(239, 68, 68, 0.1)"; trendEl.style.color = "#EF4444"; trendEl.innerText = `↘ ${Math.abs(diff)}`; noteEl.innerText = vsText; 
                    } else { 
                        trendEl.style.background = "rgba(43, 54, 116, 0.05)"; trendEl.style.color = "var(--text-main)"; trendEl.innerText = `0`; noteEl.innerText = vsText; 
                    }
                }
            ...
            if (affiliations.length === 0) {
                ...
                let headerHtml = `<thead><tr class="matrix-head-1"><th rowspan="2" class="role-cell" style="position:sticky; left:0; z-index:16; background:var(--bg-card);">Role / หน้าที่</th>`;
                affiliations.forEach(aff => {
                    headerHtml += `<th colspan="${dynamicTeams.length + 1}" class="aff-header">${aff}</th>`;
                });
                headerHtml += `<th rowspan="2" class="total-cell">Grand Total</th><th rowspan="2" class="total-cell">% Ratio</th></tr><tr class="matrix-head-2">`;
                ...
                footerHtml += `<td class="total-cell">${opsTotal}</td><td class="total-cell">100%</td></tr>`;

                // 🟢 [FIX #C1] เพิ่มแถวสรุป "พนักงานต่างชาติ / ไทย" แยกตามสังกัด
                //   (ต้องรีเฟรช/redeploy AppScript ให้ส่ง workforce[date].nat_by_aff มาด้วย; ถ้ายังไม่มีจะโชว์ '-')
                let natByAff = todayWf.nat_by_aff || {};
                const natRow = (label, key, color) => {
                    let row = `<tr><td class="role-cell" style="position:sticky; left:0; z-index:15; background:var(--bg-card); color:${color}; font-weight:700;">${label}</td>`;
                    let grand = 0;
                    affiliations.forEach(aff => {
                        dynamicTeams.forEach(() => { row += `<td style="color:var(--text-muted);">-</td>`; });
                        let n = (natByAff[aff] && natByAff[aff][key]) ? natByAff[aff][key] : 0;
                        grand += n;
                        row += `<td class="total-cell" style="color:${color};">${n > 0 ? n : '-'}</td>`;
                    });
                    row += `<td class="total-cell" style="color:${color};">${grand > 0 ? grand : '-'}</td><td class="total-cell">-</td></tr>`;
                    return row;
                };
                footerHtml += natRow('🌐 ต่างชาติ (Foreign)', 'foreign', '#3B82F6');
                footerHtml += natRow('🇹🇭 ไทย (Thai)', 'thai', '#10B981');
                footerHtml += `</tfoot>`;
                if(matrixTable) matrixTable.innerHTML = headerHtml + bodyHtml + footerHtml;
            }

            // 🟢 [FIX #8] ตารางพนักงานต่างชาติ แยกตาม "สังกัด + ตำแหน่ง" (ให้รู้ชัดว่าต่างชาติอยู่ตำแหน่งอะไร)
            const fgEl = document.getElementById('wf-foreign-detail');
            if (fgEl) {
                let fgList = (todayWf.foreign_staff || []).filter(f => window.selectedBUs.includes('ALL') || window.selectedBUs.includes(f.aff));
                if (fgList.length === 0) {
                    fgEl.innerHTML = `<div style="font-size:0.8rem; color:var(--text-muted); margin-top:12px;">🌐 ไม่พบพนักงานต่างชาติที่เข้างานในวันที่เลือก</div>`;
                } else {
                    let grp = {};
                    fgList.forEach(f => {
                        let tm = (f.team && f.team.toString().trim() !== '') ? f.team.toString().trim() : '-';
                        let k = (f.aff||'-') + '|' + tm + '|' + (f.role||'-');
                        if (!grp[k]) grp[k] = { aff: f.aff||'-', team: tm, role: f.role||'-', count: 0 };
                        grp[k].count++;
                    });
                    // [FIX] ไม่โชว์รายชื่อ (ข้อมูลเยอะ) — โชว์ สังกัด / ทีม / ตำแหน่ง / จำนวน
                    let rows = Object.values(grp).sort((a,b) => (a.aff+a.team+a.role).localeCompare(b.aff+b.team+b.role)).map(g =>
                        `<tr>
                            <td style="padding:6px 10px; border-bottom:1px solid var(--border-color);"><span class="badge info" style="font-size:10px;">${g.aff}</span></td>
                            <td style="padding:6px 10px; border-bottom:1px solid var(--border-color); text-align:center; font-weight:700;">${g.team}</td>
                            <td style="padding:6px 10px; border-bottom:1px solid var(--border-color); font-weight:700;">${g.role}</td>
                            <td style="padding:6px 10px; border-bottom:1px solid var(--border-color); text-align:center; font-weight:800; color:#3B82F6;">${g.count}</td>
                        </tr>`
                    ).join('');
                    fgEl.innerHTML = `<div style="margin-top:15px;">
                        <div style="font-size:0.8rem; font-weight:700; color:#3B82F6; margin-bottom:8px;">🌐 พนักงานต่างชาติ — แยกตามสังกัด / ทีม / ตำแหน่ง (รวม ${fgList.length} คน)</div>
                        <div style="overflow-x:auto; max-height:260px; overflow-y:auto;"><table style="width:100%; border-collapse:collapse; font-size:0.8rem;">
                            <thead><tr>
                                <th style="text-align:left; padding:6px 10px; color:var(--text-muted); font-size:0.7rem; border-bottom:2px solid var(--border-color); position:sticky; top:0; background:var(--bg-card);">สังกัด</th>
                                <th style="text-align:center; padding:6px 10px; color:var(--text-muted); font-size:0.7rem; border-bottom:2px solid var(--border-color); position:sticky; top:0; background:var(--bg-card);">ทีม</th>
                                <th style="text-align:left; padding:6px 10px; color:var(--text-muted); font-size:0.7rem; border-bottom:2px solid var(--border-color); position:sticky; top:0; background:var(--bg-card);">ตำแหน่ง</th>
                                <th style="text-align:center; padding:6px 10px; color:var(--text-muted); font-size:0.7rem; border-bottom:2px solid var(--border-color); position:sticky; top:0; background:var(--bg-card);">จำนวน (คน)</th>
                            </tr></thead>
                            <tbody>${rows}</tbody>
                        </table></div></div>`;
                }
            ...
            if (hcSummaryTable && todayWf.hc_summary_list) {
                let hcHtml = `<thead><tr>
                    <th class="role-cell" style="position:sticky; top:0; z-index:25; background:var(--bg-card);">HC Type (Lv3)</th>
                    <th class="role-cell" style="position:sticky; top:0; z-index:25; background:var(--bg-card);">Detail (Lv4)</th>
                    <th class="text-center" style="position:sticky; top:0; z-index:25; background:var(--bg-card);">Target</th>
                    <th class="text-center" style="position:sticky; top:0; z-index:25; background:var(--bg-card);">Actual</th>
                    <th class="text-center" style="position:sticky; top:0; z-index:25; background:var(--bg-card);">Absent</th>
                    <th class="text-center" style="position:sticky; top:0; z-index:25; background:var(--bg-card);">% Rate</th>
                </tr></thead><tbody>`;
...
function updateTrendChart(baseDateStr) {
    ...
    // นับกำลังพลของ "สังกัดหนึ่ง" ในวันหนึ่ง ตาม role filter ที่เลือก
    const countForAff = (dayD, aff) => {
        let m = dayD.matrix || {};
        if (!m[aff]) return 0;
        let val = 0;
        if (roleF === 'All') {
            ["Pick","RT","QC","QA","Grouping","Putaway","Receive"].forEach(r => { if (m[aff][r]) Object.values(m[aff][r]).forEach(v => val += v); });
        } else if (roleF === 'QCQA') {
            ["QC","QA"].forEach(r => { if (m[aff][r]) Object.values(m[aff][r]).forEach(v => val += v); });
        } else {
            if (m[aff][roleF]) Object.values(m[aff][roleF]).forEach(v => val += v);
        }
        return val;
    };

    // 🌟 [FIX #4] เปลี่ยนเป็น Stacked Bar แยกตามสังกัด (affiliation)
    // 1) รวบรวมรายชื่อสังกัดที่มีข้อมูลในช่วงที่แสดง (กรองตาม BU ที่เลือกด้านบน)
    let affSet = new Set();
    wfKeys.forEach(k => {
        Object.keys(globalData.workforce[k].matrix || {}).forEach(aff => {
    ...
    wfKeys.forEach(k => {
        ...
        labels.push(`${String(dObj.getDate()).padStart(2,'0')} ${months[dObj.getMonth()]}`);
    });

    // 3) 1 dataset ต่อ 1 สังกัด แล้ว stack ทับกัน (แยกสีจาก chartPalette)
    let datasets = affList.map((aff, i) => ({
        label: aff,
        data: wfKeys.map(k => countForAff(globalData.workforce[k], aff)),
    ...
    if (datasets.length === 0) {
        datasets = [{ label: 'จำนวนกำลังพล (คน)', data: wfKeys.map(() => 0), backgroundColor: primaryBlue, borderRadius: 6, stack: 'hc' }];
    }

    workforceChartInstance.data.labels = labels;
    workforceChartInstance.data.datasets = datasets;

    // เปิดโหมด stacked + แสดง legend (ชื่อสังกัด)
    if (!workforceChartInstance.options.scales) workforceChartInstance.options.scales = {};
    if (!workforceChartInstance.options.scales.x) workforceChartInstance.options.scales.x = {};
    if (!workforceChartInstance.options.scales.y) workforceChartInstance.options.scales.y = {};
    workforceChartInstance.options.scales.x.stacked = true;
    workforceChartInstance.options.scales.x.grid = { display: false };
    workforceChartInstance.options.scales.y.stacked = true;
    workforceChartInstance.options.scales.y.beginAtZero = true;
    if (!workforceChartInstance.options.plugins) workforceChartInstance.options.plugins = {};
    workforceChartInstance.options.plugins.legend = { display: true, position: 'top', labels: { boxWidth: 10, font: { size: 11 } } };

    workforceChartInstance.update();
}

function renderOnTimeSection() {
    // [FIX #5] ยอมให้ทำงานถ้ามี ontime (PTGLG/HUB) "หรือ" ontime_by_aff (แยกสังกัดจาก backend ใหม่)
    const _hasByAff = globalData.ontime_by_aff && Object.keys(globalData.ontime_by_aff).length > 0;
    if(!globalData.ontime && !_hasByAff) return;
    if(Object.keys(globalData.ontime || {}).length === 0 && !_hasByAff) return;
    if(!globalData.ontime) globalData.ontime = {};
    if(!globalData.ontime_hub) globalData.ontime_hub = {};
    ...
    // (คงไว้สำหรับกล่องสรุป + KPI card ด้านล่าง ที่ยังอิง PTGLG/HUB)
    let grouped = {};
    ...
    // ===== [FIX #5] กราฟ ON-TIME TREND: 1 เส้นต่อ 1 สังกัด + เลือกได้หลายรายการ =====
    // แหล่งข้อมูล: ใช้ ontime_by_aff จาก backend ถ้ามี, ถ้าไม่มี (backend ยังไม่อัปเดต) ก็ fallback เป็น PTGLG/HUB
    let affSource = (globalData.ontime_by_aff && Object.keys(globalData.ontime_by_aff).length > 0)
        ? globalData.ontime_by_aff
        : { 'PTGLG': globalData.ontime || {}, 'HUB': globalData.ontime_hub || {} };

    // 🟢 [FIX #D1] เพิ่มตัวเลือก "ภาพรวม (เฉลี่ยรวมทุกสังกัด)" ให้เลือกใน filter ได้ — เป็นเส้นเดียวเฉลี่ยรวม
    const OVERALL_LABEL = '📊 ภาพรวม (เฉลี่ยรวมทุกสังกัด)';
    let realAffNames = Object.keys(affSource).filter(a => a && Object.keys(affSource[a] || {}).length > 0).sort();
    let affNames = [OVERALL_LABEL, ...realAffNames];
    if (ontimeAffMS) ontimeAffMS.setOptions(affNames);
    let selRaw = ontimeAffMS ? ontimeAffMS.getSelected() : ['ALL'];
    let selectedAffs = selRaw.includes('ALL') ? affNames : selRaw.filter(a => affNames.includes(a));
    if (selectedAffs.length === 0) selectedAffs = affNames;

    // จัดกลุ่มตามช่วงเวลา (Daily/Monthly) แยก "รายสังกัดจริง" ทั้งหมดก่อน (ใช้เฉลี่ยเป็นเส้นภาพรวมด้วย)
    let groupedAff = {}; let groupTime = {};
    realAffNames.forEach(aff => {
        groupedAff[aff] = {};
        Object.keys(affSource[aff] || {}).forEach(dKey => {
            let pd = new Date(getStandardDate(dKey));
            if (isNaN(pd.getTime()) || pd.getTime() > targetTimestamp) return;
            let val = parseFloat(affSource[aff][dKey]);
            if (isNaN(val)) return;
            let gk = (otPeriod === 'Daily') ? `${String(pd.getDate()).padStart(2,'0')} ${months[pd.getMonth()]}` : `${months[pd.getMonth()]} ${pd.getFullYear()}`;
            let gt = (otPeriod === 'Daily') ? pd.getTime() : new Date(pd.getFullYear(), pd.getMonth(), 1).getTime();
            groupTime[gk] = gt;
            if (!groupedAff[aff][gk]) groupedAff[aff][gk] = { sum: 0, cnt: 0 };
            groupedAff[aff][gk].sum += val; groupedAff[aff][gk].cnt++;
        });
    });
    let allGK = Object.keys(groupTime).sort((a,b) => groupTime[a] - groupTime[b]);
    let sliceKeys = (otPeriod === 'Daily') ? allGK.slice(-14) : allGK;

    // ค่าเฉลี่ยรวมทุกสังกัด ต่อกลุ่มเวลา (สำหรับเส้น "ภาพรวม")
    const _overallAt = (gk) => {
        let s = 0, c = 0;
        realAffNames.forEach(aff => { let g = groupedAff[aff][gk]; if (g && g.cnt > 0) { s += (g.sum / g.cnt); c++; } });
        return c > 0 ? parseFloat((s / c).toFixed(2)) : null;
    };

    if (ontimeChartInstance) {
        let datasets = selectedAffs.map((aff, i) => {
            let isOverall = (aff === OVERALL_LABEL);
            let color = isOverall ? '#2B3674' : chartPalette[i % chartPalette.length];
            let dataArr = isOverall
                ? sliceKeys.map(gk => _overallAt(gk))
                : sliceKeys.map(gk => { let g = groupedAff[aff][gk]; return (g && g.cnt > 0) ? parseFloat((g.sum / g.cnt).toFixed(2)) : null; });
            return {
                label: aff,
                data: dataArr,
                isOverall: isOverall,   // 🟢 [FIX] ใช้บอก plugin ว่าเส้นนี้คือ "ภาพรวม" (ให้โชว์ป้าย % เสมอ)
                borderColor: color,
                backgroundColor: color + '22',
                fill: false, tension: 0.4,
                pointBackgroundColor: '#FFFFFF', pointBorderColor: color,
                pointRadius: isOverall ? 4 : 3, borderWidth: isOverall ? 3 : 2, spanGaps: true
            };
        });
        ontimeChartInstance.data.labels = sliceKeys;
        ontimeChartInstance.data.datasets = datasets;
    ...
    if (summaryEl && sortedG.length > 0) {
        let curr = sortedG[sortedG.length - 1];
        ...
        // [FIX #5] เอา otType (dropdown เดิม) ออก — โชว์ทั้ง PTGLG/HUB เสมอ + บอกว่ากราฟกำลังแสดงกี่สังกัด
        let affNote = (typeof selectedAffs !== 'undefined' && selectedAffs.length > 0) ? ` &nbsp;|&nbsp; <span style="color:var(--text-muted); font-size:0.85em;">กราฟแสดง ${selectedAffs.length} สังกัด</span>` : '';
        summaryEl.innerHTML = `💡 <b>ล่าสุด (${curr.label}):</b> ${t1} &nbsp;&nbsp;|&nbsp;&nbsp; ${t2} <span style="color:var(--text-muted); font-size:0.85em; margin-left:10px;">${timeWord}</span>${affNote}`;
    }
    ...
    if (ontimeEl) {
        ...
        if (trendEl && noteEl) {
            if (targetOtDateStr === null) { trendEl.style.background = "rgba(43, 54, 116, 0.05)"; trendEl.style.color = "var(--text-main)"; trendEl.innerText = "-"; noteEl.innerText = "ไม่มีข้อมูล"; } 
            else if (prevOtDateStr === null) { trendEl.style.background = "rgba(43, 54, 116, 0.05)"; trendEl.style.color = "var(--text-main)"; trendEl.innerText = "-"; noteEl.innerText = "ไม่มีข้อมูลเปรียบเทียบ"; } 
            else {
                let diff = currentOverVal - prevOverVal; 
                if (diff > 0) { trendEl.style.background = "rgba(16, 185, 129, 0.1)"; trendEl.style.color = "#10B981"; trendEl.innerText = `↗ +${diff.toFixed(2)} pp`; } 
                else if (diff < 0) { trendEl.style.background = "rgba(239, 68, 68, 0.1)"; trendEl.style.color = "#EF4444"; trendEl.innerText = `↘ ${Math.abs(diff).toFixed(2)} pp`; } 
                else { trendEl.style.background = "rgba(43, 54, 116, 0.05)"; trendEl.style.color = "var(--text-main)"; trendEl.innerText = `0 pp`; }
...
function renderClaimSection() {
    try {
        ...
        // 🌟 [FIX #6] เพิ่ม filter BU เฉพาะการ์ด CLAIM TREND (เลือกได้หลายรายการ)
        //    รวบรวมรายชื่อ BU ทั้งหมดในข้อมูลเคลมมาเป็นตัวเลือกก่อน
        let msBUs = new Set();
        Object.keys(claimsData).forEach(dKey => {
            let d = new Date(getStandardDate(dKey));
            if (!isNaN(d.getTime()) && d.getTime() <= targetTimestamp) {
                Object.keys(claimsData[dKey].bu_data || {}).forEach(bu => msBUs.add(bu));
            }
        });
        if (claimBuMS) claimBuMS.setOptions(Array.from(msBUs));
        let claimSel = claimBuMS ? claimBuMS.getSelected() : ['ALL'];
        // ถ้าเลือก BU เฉพาะบนการ์ดนี้ ให้ใช้เป็นหลัก, ถ้าเป็น All ค่อยอิง filter รวมด้านบน
        const buPass = (bu) => {
            if (!claimSel.includes('ALL')) return claimSel.includes(bu);
            return window.selectedBUs.includes('ALL') || window.selectedBUs.includes(bu);
        };
            ...
            if (sortedG.length === 0) {
                ...
                tableItems.forEach((item, index) => {
                    ...
                    tr += `<td class="total-cell" style="text-align:center; white-space:nowrap; font-size:11px; padding: 8px 4px; color:var(--danger); font-weight:700;">${fmtN(item.cost)}</td>
                           <td class="total-cell" style="text-align:center; white-space:nowrap; padding: 8px 4px; background-color:var(--bg-body);">${diffHtml}</td>
                    </tr>`;
        ...
        if (sortedG.length > 0) {
            ...
            if (trendEl && noteEl) {
                if (prevCost === null) {
                    trendEl.style.background = "rgba(43, 54, 116, 0.05)"; trendEl.style.color = "var(--text-main)"; trendEl.innerText = "-"; noteEl.innerText = "vs prev";
                } else {
                    let diff = currCost - prevCost;
                    if (diff > 0) {
                        trendEl.style.background = "rgba(239, 68, 68, 0.1)"; trendEl.style.color = "#EF4444"; trendEl.innerText = `↗ +${fmtN(diff)}`; noteEl.innerText = `vs ${prevM.label}`;
                    } else if (diff < 0) {
                        trendEl.style.background = "rgba(16, 185, 129, 0.1)"; trendEl.style.color = "#10B981"; trendEl.innerText = `↘ ${fmtN(Math.abs(diff))}`; noteEl.innerText = `vs ${prevM.label}`;
                    } else {
                        trendEl.style.background = "rgba(43, 54, 116, 0.05)"; trendEl.style.color = "var(--text-main)"; trendEl.innerText = `0`; noteEl.innerText = `vs ${prevM.label}`;
                    }
                }
            }
            if (upEl) {
    ...
    if (!locStats || locStats.total === 0) {
        ...
        if (filteredArr.length === 0) {
            ...
            filteredArr.forEach((item, idx) => {
                ...
                tbody += `<tr>
                    <td style="padding:10px; font-size:11px; font-weight:600; border-bottom:1px solid var(--border-color);">${item.bu}</td>
                    <td style="padding:10px; font-size:11px; color:var(--text-muted); border-bottom:1px solid var(--border-color);">${item.type}</td>
                    <td style="padding:10px; font-size:11px; font-weight:700; border-bottom:1px solid var(--border-color);">${item.zone}</td>
                    <td style="padding:10px; font-size:11px; text-align:center; border-bottom:1px solid var(--border-color);">${fmtN(item.checked)}</td>
                    <td style="padding:10px; font-size:11px; text-align:center; border-bottom:1px solid var(--border-color); color:${item.wrong>0?dangerRed:'inherit'}; font-weight:${item.wrong>0?'700':'400'};">${fmtN(item.wrong)}</td>
                    <td style="padding:10px; font-size:11px; text-align:center; border-bottom:1px solid var(--border-color);">
...
function renderInventorySection() {
    try {
        ...
        // 🌟 [FIX #7] เตรียมตัวเลือก Location Type (ดึงจาก loctype_data ที่ backend ส่งมาใหม่)
        let allLocTypes = new Set();
        Object.keys(invData).forEach(m => { let lt = invData[m] && invData[m].loctype_data; if (lt) Object.keys(lt).forEach(t => allLocTypes.add(t)); });
        if (invLocTypeMS) invLocTypeMS.setOptions(Array.from(allLocTypes));
        let locSel = invLocTypeMS ? invLocTypeMS.getSelected() : ['ALL'];
        const locTypeActive = !locSel.includes('ALL') && allLocTypes.size > 0;
        ...
        Object.keys(invData).forEach(mLabel => {
            ...
            if (parts.length === 2) {
                ...
                parsedData.push({
                    ...
                    loctype_data: monthGroup.loctype_data || null // [FIX #7]
                });
            }
        });
        ...
        if (inventoryChartInstance) {
            ...
            validData.forEach(item => {
                ...
                if (locTypeActive) {
                    // 🌟 [FIX #7] เลือก Location Type อยู่ -> คิดความแม่นยำจาก loctype_data (รวมทุก BU) ตามประเภทที่เลือก
                    let oh = 0, df = 0, cnt = 0;
                    if (item.loctype_data) {
                        locSel.forEach(t => { let d = item.loctype_data[t]; if (d) { oh += (d.onhand || 0); df += (d.diff || 0); cnt += (d.count || 0); } });
                    }
                    let v = null;
                    if (cnt > 0) v = oh > 0 ? Math.max(0, (1 - (df / oh)) * 100) : (df === 0 ? 100 : 0);
                    data.push(v !== null ? parseFloat(v.toFixed(2)) : null);
                } else if (invFilterBU === 'ALL' || !window.selectedBUs.includes('ALL')) {
            ...
            inventoryChartInstance.data.datasets[0].label = locTypeActive ? `Accuracy % (${locSel.join(', ')})` : 'Accuracy %';
        ...
        if (validData.length > 0) {
            ...
            if (trendEl && noteEl) {
                if (currPct === null || prevPct === null) {
                    ...
                    if (diff > 0) {
                        trendEl.style.background = "rgba(16, 185, 129, 0.1)"; trendEl.style.color = "#10B981"; trendEl.innerText = `↗ +${diff.toFixed(2)} pp`; noteEl.innerText = `vs ${prevM.label}`;
                    } else if (diff < 0) {
                        trendEl.style.background = "rgba(239, 68, 68, 0.1)"; trendEl.style.color = "#EF4444"; trendEl.innerText = `↘ ${Math.abs(diff).toFixed(2)} pp`; noteEl.innerText = `vs ${prevM.label}`;
                    } else {
                        trendEl.style.background = "rgba(43, 54, 116, 0.05)"; trendEl.style.color = "var(--text-main)"; trendEl.innerText = `0 pp`; noteEl.innerText = `vs ${prevM.label}`;
                    }
                }
            }
            if (upEl) {
        ...
        // 🟢 [FIX #E1] อัปเดตกล่องสรุป %Accuracy (ภาพรวม + ตาม Location Type, ทั้งรายวัน/รายเดือน แยก BU)
...
// 🟢 [FIX #E1] กล่องสรุปความแม่นยำสต๊อก: ภาพรวม + ต่อ BU + ตาม Location Type (รายเดือนล่าสุด & รายวันล่าสุด)
function renderInventorySummary() {
    const box = document.getElementById('inv-summary-box');
    if (!box) return;
    if (!globalData.inventory || Object.keys(globalData.inventory).length === 0) { box.innerHTML = '💡 ไม่มีข้อมูลความแม่นยำสต๊อก'; return; }

    const months = ["Jan","Feb","Mar","Apr","May","Jun","Jul","Aug","Sep","Oct","Nov","Dec"];
    const dpVal = document.getElementById('date-picker')?.value || new Date().toISOString().split('T')[0];
    const targetTs = new Date(dpVal).setHours(23,59,59,999);

    const selBUs = window.selectedBUs || ['ALL'];
    const buPass = (bu) => { let cb = (bu||'').toUpperCase(); if (cb.includes('TOTAL')||cb.includes('OVERALL')) return false; return selBUs.includes('ALL') || selBUs.includes(bu); };
    let locSel = (invLocTypeMS && invLocTypeMS.getSelected) ? invLocTypeMS.getSelected() : ['ALL'];
    const typePass = (t) => locSel.includes('ALL') || locSel.includes(t);

    const acc = (oh, df) => oh > 0 ? Math.max(0, (1 - df/oh)*100) : (df === 0 ? 100 : null);
    const pill = (v) => (v === null || v === undefined) ? '<span style="color:var(--text-muted);">-</span>'
        : `<span style="display:inline-block; background:${v>=99?'#dcfce7':'#fee2e2'}; color:${v>=99?'#166534':'#991b1b'}; padding:1px 7px; border-radius:10px; font-weight:700;">${v.toFixed(2)}%</span>`;

    let monthArr = Object.keys(globalData.inventory).map(m => {
        let p = m.split('-'); if (p.length !== 2) return null;
        return { label: m, ts: new Date(`${p[0]} 1, 20${p[1]}`).getTime(), data: globalData.inventory[m] };
    }).filter(x => x && !isNaN(x.ts) && x.ts <= targetTs).sort((a,b) => a.ts - b.ts);

    let dayArr = Object.keys(globalData.inventory_daily || {}).map(d => {
        let dObj = new Date(getStandardDate(d));
        return isNaN(dObj.getTime()) ? null : { label: d, ts: dObj.getTime(), data: globalData.inventory_daily[d] };
    }).filter(x => x && x.ts <= targetTs).sort((a,b) => a.ts - b.ts);

    let monthG = monthArr.length ? monthArr[monthArr.length - 1] : null;
    let dayG = dayArr.length ? dayArr[dayArr.length - 1] : null;

    // รวมรายชื่อ BU (ผ่านฟิลเตอร์) จากเดือนล่าสุด & วันล่าสุด เพื่อทำเป็นคอลัมน์ของตาราง
    let buSet = new Set();
    [monthG, dayG].forEach(g => { if (g && g.data.bu_data) Object.keys(g.data.bu_data).forEach(bu => { if (buPass(bu)) buSet.add(bu); }); });
    let buCols = Array.from(buSet).sort();

    const tdMuted = `<td style="text-align:center; padding:6px 8px; border-bottom:1px solid var(--border-color); color:var(--text-muted);">-</td>`;
    const cellPill = (v) => `<td style="text-align:center; padding:6px 8px; border-bottom:1px solid var(--border-color);">${pill(v)}</td>`;

    // แถวของช่วงหนึ่ง: "ภาพรวม" + แถวย่อยตาม Location Type
    const rowsFor = (periodLabel, g) => {
        if (!g) return `<tr><td style="padding:6px 10px; border-bottom:1px solid var(--border-color);">${periodLabel}</td><td colspan="${buCols.length+1}" style="padding:6px 10px; border-bottom:1px solid var(--border-color); color:var(--text-muted);">ไม่มีข้อมูล</td></tr>`;
        let bd = g.data.bu_data || {};
        let oh=0, df=0, cnt=0; let cells = '';
        buCols.forEach(bu => {
            let d = bd[bu];
            if (d && d.count) { oh+=d.onhand; df+=d.diff; cnt+=d.count; cells += cellPill(acc(d.onhand, d.diff)); }
            else cells += tdMuted;
        });
        let html = `<tr style="background:var(--bg-body);"><td style="padding:6px 10px; border-bottom:1px solid var(--border-color); font-weight:700; position:sticky; left:0; background:var(--bg-body);">${periodLabel} · ภาพรวม</td>${cells}<td style="text-align:center; padding:6px 8px; border-bottom:1px solid var(--border-color); font-weight:800;">${pill(cnt>0?acc(oh,df):null)}</td></tr>`;

        let lt = g.data.loctype_data || {};
        Object.keys(lt).sort().forEach(t => {
            if (!typePass(t)) return;
            let tcells = ''; let toh=0, tdf=0, tcnt=0; let hasBu=false;
            buCols.forEach(bu => {
                let dd = (lt[t].bu && lt[t].bu[bu]) ? lt[t].bu[bu] : null;
                if (dd && dd.count) { toh+=dd.onhand; tdf+=dd.diff; tcnt+=dd.count; hasBu=true; tcells += cellPill(acc(dd.onhand, dd.diff)); }
                else tcells += tdMuted;
            });
            if (!hasBu && lt[t].count) { toh=lt[t].onhand; tdf=lt[t].diff; tcnt=lt[t].count; }
            if (tcnt > 0) html += `<tr><td style="padding:6px 10px 6px 22px; border-bottom:1px solid var(--border-color); color:var(--text-muted); position:sticky; left:0; background:var(--bg-card);">↳ ${t}</td>${tcells}<td style="text-align:center; padding:6px 8px; border-bottom:1px solid var(--border-color); font-weight:700;">${pill(acc(toh,tdf))}</td></tr>`;
        });
        return html;
    };

    let mTitle = monthG ? `📅 รายเดือน (${monthG.label})` : '📅 รายเดือน';
    let dTitle = dayG ? `🗓️ รายวัน (${String(new Date(dayG.ts).getDate()).padStart(2,'0')} ${months[new Date(dayG.ts).getMonth()]})` : '🗓️ รายวัน';

    let thead = `<thead><tr>
        <th style="text-align:left; padding:8px 10px; position:sticky; left:0; background:var(--bg-card); z-index:2; border-bottom:2px solid var(--border-color);">ช่วง / ขอบเขต</th>
        ${buCols.map(bu => `<th style="text-align:center; padding:8px; border-bottom:2px solid var(--border-color);">${bu}</th>`).join('')}
        <th style="text-align:center; padding:8px; border-bottom:2px solid var(--border-color);">ภาพรวม</th>
    </tr></thead>`;

    box.innerHTML = `<div style="overflow-x:auto;"><table style="width:100%; border-collapse:collapse; font-size:0.8rem; min-width:520px;">${thead}<tbody>${rowsFor(mTitle, monthG)}${rowsFor(dTitle, dayG)}</tbody></table></div>
        <div style="margin-top:6px; font-size:0.72rem; color:var(--text-muted);">* กรองตาม BU และ Location Type ที่เลือกด้านบน · ตัวเลข = %Accuracy (เขียว ≥ 99%) · แถว ↳ = แยกตาม Location Type</div>`;
}
    ...
    try {
        ...
        if (carrierTableEl && validData.length > 0) {
            ...
            if (carriers.length === 0 || sortedStatuses.length === 0) {
                ...
                sortedStatuses.forEach(s => {
                    tfoot += `<td style="text-align:center; white-space:nowrap; font-size:11px; padding: 10px; font-weight:800; border-top: 1px solid var(--border-color); position:sticky; bottom:0; z-index:25; background:var(--bg-card);">${grandTotals[s] > 0 ? fmtN(grandTotals[s]) : '-'}</td>`;
                });
                tfoot += `<td class="total-cell" style="text-align:center; white-space:nowrap; font-size:12px; padding: 10px; font-weight:800; color:var(--accent-primary); border-top: 1px solid var(--border-color); position:sticky; bottom:0; z-index:25; background:var(--bg-card);">${fmtN(grandTotals.total)}</td></tr></tfoot>`;
...
function renderProductivitySection() {
    try {
        ...
        let pZones = globalData.prod_zone || {};   // 🟢 [FIX] PRODUCTIVITY BY ZONE (S/T/U)
        ...
        validDates.forEach(item => {
            ...
            if (!grouped[groupKey]) {
                grouped[groupKey] = { label: groupKey, time: item.time, user_picks: 0, user_hours: 0, areas: {}, zones: {}, users: {}, activePickers: new Set() };
            }

            // 🟢 [FIX] รวมข้อมูล PRODUCTIVITY BY ZONE ต่อกลุ่มเวลา (Productivity ต่อโซน = ค่าเฉลี่ยของคอลัมน์ T)
            if (pZones[dKey]) {
        ...
        // 🟢 [FIX #F1] target/cost ยึดค่าตามวันที่ (carry-forward) — คำนวณ dateStr ต่อกลุ่มจาก group.time
        ...
        // 🟢 [FIX] ภาพรวม (ALL) ไม่นับรายการ Bulk/Error (ชั่วโมงน้อยผิดปกติ < 0.08 หรือ UPH > 600) เข้ามาถ่วงน้ำหนัก
        ...
        const noteBulk = `<tr><td colspan="99" style="padding:6px 10px; color:var(--text-muted); font-size:0.7rem;">* ค่าเฉลี่ย/ภาพรวม ไม่รวมรายการ Bulk/Error (ชั่วโมงผิดปกติ หรือ UPH &gt; 600)</td></tr>`;

        if (productivityChartInstance) {
            let labels = [], dataActual = [], dataBackground = [], dataTarget = [], bgColorsActual = [], bgColorsBg = [], customPicks = [];
            let maxUPH = 0;
            let maxTargetSlice = currentTarget;

            chartSlice.forEach(g => {
                let uph = 0;
                if (selectedArea === 'ALL') {
                    let nb = nonBulkTotals(g);
                    uph = nb.hours > 0 ? Math.round(nb.picks / nb.hours) : 0;
            ...
            chartSlice.forEach(g => {
                labels.push(g.label);
                let uph = 0, picks = 0;
                let gt = getTarget(selectedArea, groupDateStr(g)); // เป้าของ "วันนั้น" (ย้อนหลังได้)

                if (selectedArea === 'ALL') {
                    let nb = nonBulkTotals(g);
                    uph = nb.hours > 0 ? Math.round(nb.picks / nb.hours) : 0;
                    picks = Math.round(nb.picks);
                } else if (g.areas[selectedArea]) {
                    uph = g.areas[selectedArea].hours > 0 ? Math.round(g.areas[selectedArea].picks / g.areas[selectedArea].hours) : 0;
        ...
        // 🟢 [FIX #F1] COST BASE ยึดค่าตามวันที่ (carry-forward) — คิด hourlyRate ต่อวันได้
        ...
        if (summaryEl) {
            ...
            if (selectedArea === 'ALL') {
                let nb = nonBulkTotals(latest);
                uph = nb.hours > 0 ? Math.round(nb.picks / nb.hours) : 0;
                pks = nb.picks; hrs = nb.hours;
        ...
        if (areaTableEl) {
            // 🟢 [FIX] PRODUCTIVITY BY ZONE: Picking Zone (คอลัมน์ S) | Area (U) | Productivity (ค่าเฉลี่ยของ T ที่คำนวณไว้แล้ว)
            ...
            let html = `<thead><tr>
                <th style="position:sticky; top:0; background:var(--bg-card); padding:12px; font-size:11px; text-align:left; border-bottom: 1px solid var(--border-color);">Picking Zone</th>
                <th class="text-center" style="position:sticky; top:0; background:var(--bg-card); padding:12px; font-size:11px; border-bottom: 1px solid var(--border-color);">Area</th>
                <th class="text-center" style="position:sticky; top:0; background:var(--bg-card); padding:12px; font-size:11px; border-bottom: 1px solid var(--border-color);">Productivity (UPH)</th>
            ...
            if(zoneNames.length === 0) {
                html += `<tr><td colspan="5" class="text-center" style="padding:20px; color:var(--text-muted); font-size:11px;">ไม่มีข้อมูล Zone ในช่วงที่เลือก</td></tr>`;
            } else {
                zoneNames.forEach(z => {
                    let zd = zoneObj[z];
                    ...
                    let gap = prod - trg;
                    let cost = prod > 0 ? (hourlyRate / prod) : 0;

                    html += `<tr>
                        <td style="padding:10px 12px; border-bottom: 1px dashed var(--border-color);"><b style="color:var(--text-main); font-size:11px;">${z}</b></td>
                        <td class="text-center" style="padding:10px 12px; border-bottom: 1px dashed var(--border-color); font-size:11px; color:var(--text-muted);">${zd.area || '-'}</td>
                        <td class="text-center" style="padding:10px 12px; border-bottom: 1px dashed var(--border-color);"><span class="badge" style="background:${prod>=trg?'#dcfce7':'#fee2e2'}; color:${prod>=trg?'#166534':'#991b1b'}">${prod}</span></td>
                ...
                html += `<tr><td colspan="5" style="padding:6px 10px; color:var(--text-muted); font-size:0.7rem;">* Productivity = ค่าเฉลี่ยของค่าที่คำนวณไว้แล้วในชีต (คอลัมน์ T) ต่อโซน</td></tr>`;
            }
            areaTableEl.innerHTML = html + "</tbody>";
            let titleEl = document.getElementById('prod-area-title');
            if(titleEl) titleEl.innerText = `PRODUCTIVITY BY ZONE (${latest.label})`;
        }
        ...
        if (userTableEl) {
            ...
            if(users.length === 0) {
                html += `<tr><td colspan="5" class="text-center" style="padding:20px; color:var(--text-muted); font-size:11px;">ไม่มีข้อมูลพนักงาน</td></tr>`;
            } else {
                users.forEach(u => {
            ...
            userTableEl.innerHTML = html + noteBulk + "</tbody>";
        ...
        if (overallTableEl) {
            ...
            sortedGroups.slice().reverse().forEach(item => {
                let nb = nonBulkTotals(item); // ไม่รวม Bulk/Error
                let uph = nb.hours > 0 ? Math.round(nb.picks / nb.hours) : 0;
                let _ds = groupDateStr(item);
                let trg = getTarget(selectedArea, _ds);
                ...
                html += `<tr>
                    ...
                    <td class="text-center" style="padding:10px 12px; border-bottom: 1px dashed var(--border-color); font-weight:600; font-size:11px;">${fmtN(Math.round(nb.picks))}</td>
            ...
            overallTableEl.innerHTML = html + noteBulk + "</tbody>";
        }
...
function getTarget(areaName, dateStr) {
    // 🟢 [FIX #F1] ใช้ค่า UPH target ที่ "มีผล ณ วันที่" (carry-forward เหมือน Cap) — ถ้าไม่ส่ง dateStr จะยึดวันบน date-picker
    let eff = getEffectiveUphCost(dateStr);
...
// 🟢 [FIX #F1] บันทึก UPH TARGET + COST BASE ลง BigQuery ผ่าน backend (เหมือน Cap) — แก้ย้อนหลังตามวันที่ได้
async function saveTargets() {
    ...
    try {
        ...
        console.error('Save UPH/Cost Error:', err);
...
document.getElementById('ffm-bu-filter')?.addEventListener('change', initFulfillmentRealtime);
